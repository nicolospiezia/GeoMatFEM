...
% Save command: 
filename = ['FS1CC', '_', num2str(dim),'D', '_', num2str(nonlin)];
% Material properties
P0=-0.05;                        pi=-0.05; %/exp(-1);
OCR=1.0;                         Pc0=OCR*pi;
nu=0.261;                        n=0.36;%e=2.36;
k=0.02;                          ks=k/(1+k);
lamda=0.15;                       lamdas=lamda/(1+lamda);         
epsev0=k*log(pi/P0);             %K=-pi*(1+e)/ks; 
epsev0s=ks*log(pi/P0);           mu0=5.0;%mu0=K*(3/2)*(1-2*nu)/(2*(1+nu));
...
if nonlin == 0 
   %G= [CC rhos   mu0 alfa k   lamda   M   Po Pc0 epsev0  - - - D n rhow   muw kx/rg ky/rg kz/rg]
    G= [4  2.7e-9 mu0 0.0  ks  lamdas  1.0 P0 Pc0 epsev0s 0 0 0 0 n 1.0e-9 0.0 1.0   1.0   1.0];  
else
   %G= [CC rhos   mu0 alfa k  lamda  M   Po Pc0 epsev0 - - - D n rhow   muw kx/rg ky/rg kz/rg]
    G= [4  2.7e-9 mu0 0.0  k  lamda  1.0 P0 Pc0 epsev0 0 0 0 0 n 1.0e-9 0.0  1.0   1.0   1.0]; 
end
...
nmax = 30;                 % Max number of load steps
...
%analisi
incrf = (1.0/nmax:1/nmax:1.0)*0.1;% <-----pressione finale su brick 0,1Mpa
incrv = ones(1,nmax);
