...
% Save command: 
filename = ['cilindro', '_', num2str(dim),'D', '_', num2str(nonlin), '_CC_iso'];

% Material properties
P0=-0.010;                       pi=-0.010; %/exp(-1);
OCR=1.0;                         Pc0=OCR*pi;
nu=0.261;                        n=0.0;%e=2.36;
k=0.02;                          ks=k/(1+k);
...
if nonlin == 0 
   %G= [CC rhos   mu0 alfa k   lamda   M   Po Pc0 epsev0  - - - D n rhow   muw kx/rg ky/rg kz/rg]
    G= [4  0.0e-9 mu0 0.0  ks  lamdas  1.0 P0 Pc0 epsev0s 0 0 0 0 n 0.0e-9 0.0 1.0   1.0   1.0];  
else
   %G= [CC rhos   mu0 alfa k  lamda  M   Po Pc0 epsev0 - - - D n rhow   muw kx/rg ky/rg kz/rg]
    G= [4  0.0e-9 mu0 0.0  k  lamda  1.0 P0 Pc0 epsev0 0 0 0 0 n 0.0e-9 0.0  1.0   1.0   1.0]; 
end
...
incrv =zeros(1,nmax);
incrf = (1.0/nmax:1/nmax:1.0)*0.2;% <-----pressione finale su brick 0,2Mpa  
